...
def crop_face_box(frame, box, face_size=160):
    ...
    crop_img = frame[startY:startY + endY, startX:startX + endX]
    transform = albu.Compose(
        [albu.LongestMaxSize(160), albu.PadIfNeeded(160, 160, border_mode=cv2.BORDER_CONSTANT, value=[0, 0, 0])])
    resized = transform(image=crop_img)

    #
    # if side_size < 0:
    #     side_size = abs(side_size)
    #     startX = endX
    #     startY = endY
    ...
    # cv2.imwrite("B001.jpg", cropped_face)
    return preprocess_image(resized['image'], do_random_crop=False, do_random_flip=False, image_size=160).reshape(
        (1, 160, 160, 3))
